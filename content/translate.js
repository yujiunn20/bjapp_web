(function () {
  const dictionaries = {
    en: {
      ...
      "變異數": "Variance",
      "優勢很小，下注級距會放大短期波動": "The Edge Is Small, and Bet Ramps Increase Swings",
      "電影常讓人覺得算牌可以短時間大賺，但現實不是這樣。即使一套策略在長期是正 EV，短期仍然可能大輸。Flat betting 的 blackjack 和許多賭場遊戲相比不一定是高變異數遊戲；算牌真正困難的地方，是優勢通常很小，而 bet ramp / bet spread 會在高 TC 時提高下注，讓短期盈虧波動變大。真正的結果通常需要非常多手牌才會接近期望值。": "Movies often make counting look like a way to win a lot of money quickly. Real play is not like that. Even if a strategy is positive EV in the long run, it can still lose badly in the short term. Flat-bet blackjack is not necessarily a high-variance game compared with many casino games; the harder part for card counting is that the edge is usually small, while a bet ramp or bet spread raises the wager in high-TC spots and makes short-term swings larger. The results usually need many hands before they begin to resemble the expected value.",
    ...
    ja: {
      ...
      "變異數": "分散",
      "優勢很小，下注級距會放大短期波動": "優位は小さく、ベットランプが短期変動を大きくする",
      "電影常讓人覺得算牌可以短時間大賺，但現實不是這樣。即使一套策略在長期是正 EV，短期仍然可能大輸。Flat betting 的 blackjack 和許多賭場遊戲相比不一定是高變異數遊戲；算牌真正困難的地方，是優勢通常很小，而 bet ramp / bet spread 會在高 TC 時提高下注，讓短期盈虧波動變大。真正的結果通常需要非常多手牌才會接近期望值。": "映画では、カウントで短期間に大きく稼げるように見えることがあります。しかし現実は違います。長期的に positive EV の戦略でも、短期では大きく負けることがあります。Flat betting の blackjack は、多くのカジノゲームと比べて必ずしも高分散とは限りません。カードカウントで難しいのは、優位が通常小さい一方で、bet ramp / bet spread によって高 TC の場面でベットが大きくなり、短期の振れ幅が大きくなることです。結果が期待値に近づくには非常に多くのハンドが必要です。",
      ...
      "Blackjack 的結果仍然會有明顯短期波動，尤其是算牌策略搭配 bet ramp 時。即使跑很多局，結果仍然不會完全等於理論機率。不過從另一個角度看，這也比較接近真實狀況：真實遊戲不是每一段結果都剛好等於 EV，而是會有明顯波動。你可以多跑幾次同樣設定，觀察每次結果之間的差異，理解短期結果和長期期望值之間的距離。": "Blackjack の結果には明確な短期変動が残り、特にカウント戦略で bet ramp を使う場合はそれが目立ちます。多くのラウンドを走らせても、結果が理論確率と完全に一致するわけではありません。ただし別の見方をすると、これは実戦にも近い状態です。実際のプレイでは、すべての区間が EV 通りになるわけではなく、変動が起こります。同じ設定を何度か走らせることで、短期結果と長期期待値の距離を確認できます。",
      ...
      "下方也會保留每個 Play 的簡單總結，包含 Final Net、Rounds、Hands 和 End Reason。這個設計主要是用來輔助停損停利與資金管理決策，比單純看大量 Round Stats 更接近現實。算牌模擬仍然可能和理論 EV 有明顯差距，因為短期波動仍然重要，尤其在 bet spread 較大時更明顯。因此同一套策略建議多跑幾次，觀察結果分布是否穩定。": "下部には各 Play の簡単な概要も残ります。内容は Final Net、Rounds、Hands、End Reason です。この設計は主に損切り、利確、資金管理の判断を助けるためのもので、大量の Round Stats だけを見るよりも現実に近い形です。カウントのシミュレーションは、短期変動が残るため理論 EV と大きく違うことがあります。特に bet spread が大きい場合はその差が目立ちます。同じ戦略を何度か走らせ、結果分布が安定しているか確認するのが有用です。",
  ...
  Object.assign(dictionaries.en, {
    ...
    "詳細統計會進一步拆解各座位或策略的結果，方便比較 insurance、surrender、split 次數、輸贏比例等細項差異。": "Detailed stats break down each seat or strategy further, making it easier to compare details such as insurance, surrender, split counts, and win/loss ratios.",
  ...
  Object.assign(dictionaries.ja, {
    ...
    "詳細統計會進一步拆解各座位或策略的結果，方便比較 insurance、surrender、split 次數、輸贏比例等細項差異。": "詳細統計では各座席や戦略の結果をさらに分解し、insurance、surrender、split 回数、勝敗比率などの細かな違いを比較できます。",
  ...
  Object.assign(dictionaries.en, {
    ...
    "Insurance 的下注額通常以原下注的一半計算，賠率是 2:1；但在大多數情況下，保險是不利下注，只有牌堆中 10 點牌比例偏高時，才可能變成合理選擇。": "Insurance is usually calculated as half of the original bet and pays 2:1. It is usually a bad bet, and becomes reasonable only when the deck is rich enough in 10-value cards.",
  ...
  Object.assign(dictionaries.ja, {
    ...
    "Insurance 的下注額通常以原下注的一半計算，賠率是 2:1；但在大多數情況下，保險是不利下注，只有牌堆中 10 點牌比例偏高時，才可能變成合理選擇。": "Insurance のベット額は通常、元のベットの半分として計算され、配当は 2:1 です。多くの場合は不利なベットで、10 点カードの割合が十分高いときだけ合理的になります。",
  ...
  Object.assign(dictionaries.en, {
    ...
    "Insurance 的 EV 與波動": "Insurance EV and Variance",
    "Insurance 本質上是在押莊家的底牌是不是 10 點牌。因為賠率是 2:1，只有當剩餘牌堆中 10 點牌比例高到超過損益平衡點時，insurance 才值得買。這也是為什麼 Hi-Lo 常用 TC >= +3 作為買保險門檻。": "Insurance is essentially a bet that the dealer's hole card is a 10-value card. Because it pays 2:1, insurance is worth taking only when the remaining shoe has enough 10-value cards to pass the break-even point. This is why Hi-Lo commonly uses TC >= +3 as the insurance index.",
    "長期來看，正確買 insurance 的主要價值是增加 EV：在高 TC 時，這個邊注可能從一般情況下的負期望值，變成有利於玩家的正期望值。": "Over the long run, the main value of taking insurance correctly is increasing EV: at high TC, this side bet can move from negative expectation in normal situations to positive expectation for the player.",
    "Insurance 對波動也有影響。它可以在莊家 blackjack 時抵消主注損失，特別是高 TC 時常伴隨較大的下注，因此有機會讓結果分布更平滑、減少大注遇到莊家 blackjack 的下檔衝擊。不過 insurance 本身仍是額外邊注，所以比較精確的理解是：正確使用 insurance 主要是提高 EV，同時可能讓部分高風險情境的波動更平滑，而不是保證每一種統計口徑下的 variance 都下降。": "Insurance also affects variance. It can offset the main-bet loss when the dealer has blackjack, especially because high TC often comes with larger bets, so it can make the result distribution smoother and reduce the downside impact of a large bet running into dealer blackjack. However, insurance is still an additional side bet, so the more precise interpretation is: correct insurance mainly improves EV, while it may smooth variance in some high-risk situations, rather than guaranteeing lower variance under every statistical view.",
    "下面這組模擬使用 Hi-Lo，Bet Ramp 為 4/8/12/16/20，共跑 500000 次。Threshold 的 TC >= k 採用 k = 3，用來觀察 TC >= 3 區間的表現。": "The simulation below uses Hi-Lo with a 4/8/12/16/20 Bet Ramp over 500,000 runs. The threshold TC >= k uses k = 3 to examine performance in the TC >= 3 range.",
    "Seat 1 使用 Hi-Lo、Bet Ramp 4/8/12/16/20 並買 insurance 的 500000 次模擬結果": "Seat 1 500,000-run simulation using Hi-Lo, Bet Ramp 4/8/12/16/20, with insurance",
    "Seat 1：有買 insurance": "Seat 1: with insurance",
    "Seat 2 使用 Hi-Lo、Bet Ramp 4/8/12/16/20 但不買 insurance 的 500000 次模擬結果": "Seat 2 500,000-run simulation using Hi-Lo, Bet Ramp 4/8/12/16/20, without insurance",
    "Seat 2：沒有買 insurance": "Seat 2: without insurance",
    "從上圖可以看到，Seat 1 有買保險的淨利是正的；在這次模擬中，買保險的 ROI 是 4.17%。和 Seat 2 比較，Seat 1 的 EV / 100 較高（35.16 vs. 23.32），SD / 100 也較小（593.12 vs. 595.6），不過 SD 的差別不大。這組結果符合前面的概念：正確使用 insurance 主要能提高 EV，同時可能讓高 TC、大注遇到莊家 blackjack 的波動稍微更平滑。": "From the charts above, Seat 1 with insurance has positive net profit; in this simulation, the ROI from insurance is 4.17%. Compared with Seat 2, Seat 1 has higher EV / 100 (35.16 vs. 23.32) and slightly lower SD / 100 (593.12 vs. 595.6), though the SD difference is small. This matches the idea above: correct insurance mainly improves EV, while it may slightly smooth the volatility of high-TC, large-bet situations when the dealer has blackjack.",
  ...
  Object.assign(dictionaries.ja, {
    ...
    "Insurance 的 EV 與波動": "Insurance の EV と変動",
    "Insurance 本質上是在押莊家的底牌是不是 10 點牌。因為賠率是 2:1，只有當剩餘牌堆中 10 點牌比例高到超過損益平衡點時，insurance 才值得買。這也是為什麼 Hi-Lo 常用 TC >= +3 作為買保險門檻。": "Insurance は本質的に、ディーラーのホールカードが 10 点カードかどうかに賭けるものです。配当が 2:1 なので、残りのシューにある 10 点カードの割合が損益分岐点を超えるときだけ取る価値があります。これが Hi-Lo で TC >= +3 が insurance の基準としてよく使われる理由です。",
    "長期來看，正確買 insurance 的主要價值是增加 EV：在高 TC 時，這個邊注可能從一般情況下的負期望值，變成有利於玩家的正期望值。": "長期的には、正しく insurance を取る主な価値は EV を上げることです。高 TC では、このサイドベットが通常の負の期待値から、プレイヤーに有利な正の期待値に変わる可能性があります。",
    ...
    "下面這組模擬使用 Hi-Lo，Bet Ramp 為 4/8/12/16/20，共跑 500000 次。Threshold 的 TC >= k 採用 k = 3，用來觀察 TC >= 3 區間的表現。": "下のシミュレーションは Hi-Lo と Bet Ramp 4/8/12/16/20 を使い、500,000 回実行したものです。Threshold の TC >= k は k = 3 に設定し、TC >= 3 の区間を確認しています。",
    "Seat 1 使用 Hi-Lo、Bet Ramp 4/8/12/16/20 並買 insurance 的 500000 次模擬結果": "Seat 1: Hi-Lo、Bet Ramp 4/8/12/16/20、insurance ありの 500,000 回シミュレーション結果",
    "Seat 1：有買 insurance": "Seat 1：insurance あり",
    "Seat 2 使用 Hi-Lo、Bet Ramp 4/8/12/16/20 但不買 insurance 的 500000 次模擬結果": "Seat 2: Hi-Lo、Bet Ramp 4/8/12/16/20、insurance なしの 500,000 回シミュレーション結果",
    "Seat 2：沒有買 insurance": "Seat 2：insurance なし",
    "從上圖可以看到，Seat 1 有買保險的淨利是正的；在這次模擬中，買保險的 ROI 是 4.17%。和 Seat 2 比較，Seat 1 的 EV / 100 較高（35.16 vs. 23.32），SD / 100 也較小（593.12 vs. 595.6），不過 SD 的差別不大。這組結果符合前面的概念：正確使用 insurance 主要能提高 EV，同時可能讓高 TC、大注遇到莊家 blackjack 的波動稍微更平滑。": "上の図を見ると、insurance ありの Seat 1 は純利益がプラスです。このシミュレーションでは、insurance の ROI は 4.17% です。Seat 2 と比べると、Seat 1 は EV / 100 が高く（35.16 vs. 23.32）、SD / 100 もやや小さくなっています（593.12 vs. 595.6）が、SD の差は大きくありません。この結果は前述の考え方と一致します。正しい insurance は主に EV を高め、同時に高 TC・大きなベットでディーラー blackjack に当たる場面の変動を少し滑らかにする可能性があります。",
    ...
    "= +2 時 Stand": ">= +2 で Stand",
    "= +5 時 Split": ">= +5 で Split",
    "= 0 時 Double": ">= 0 で Double",
    "= +4 時 Double": ">= +4 で Double",
    "= +1 時 Double": ">= +1 で Double",
    "= +3 時 Double": ">= +3 で Double",
    "保險": "インシュランス",
  ...
  Object.assign(dictionaries.en, {
    ...
    "不是。模擬結果只是結果分布中的一個樣本。即使某個策略在模擬中有較好的 EV 或 ROI，實際結果仍可能因短期波動、桌規、穿透率、桌限、資金、執行穩定度、bet spread 等因素而不同。統計結果適合用來理解和比較，不應被當成實際獲利保證。": "No. A simulation result is only one sample from a distribution. Even if a strategy has better EV or ROI in simulation, real outcomes can still differ because of short-term swings, table rules, penetration, table limits, bankroll, execution consistency, bet spread, and other factors. Statistics are useful for understanding and comparison, not as a guarantee of real profit.",
  ...
  Object.assign(dictionaries.ja, {
    ...
    "不是。模擬結果只是結果分布中的一個樣本。即使某個策略在模擬中有較好的 EV 或 ROI，實際結果仍可能因短期波動、桌規、穿透率、桌限、資金、執行穩定度、bet spread 等因素而不同。統計結果適合用來理解和比較，不應被當成實際獲利保證。": "いいえ。シミュレーション結果は、結果分布から得られた 1 つのサンプルにすぎません。ある戦略がシミュレーション上で良い EV や ROI を示しても、実際の結果は短期変動、ルール、ペネトレーション、テーブルリミット、資金、実行精度、bet spread などによって変わります。統計結果は理解と比較のために使うもので、実際の利益を保証するものではありません。",
    ...
    "不是。這個 app 和網站主要是用來學習 blackjack 機率、策略、算牌概念和訓練反應，不提供真錢賭博功能，也不把算牌包裝成穩賺方法。使用者仍應遵守所在地法律和平台規範，並理解 blackjack 涉及風險、短期波動和資金壓力。": "いいえ。このアプリとウェブサイトは主に blackjack の確率、戦略、カードカウント概念、反応練習を学ぶためのものです。現金賭博機能は提供せず、カウントを確実に稼げる方法として見せるものでもありません。ユーザーは所在地の法律とプラットフォーム規約を守り、blackjack にはリスク、短期変動、資金面のプレッシャーがあることを理解する必要があります。"
  });
